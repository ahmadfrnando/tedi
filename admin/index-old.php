...
$kategori = mysqli_query($conn, "SELECT * FROM ref_kategori ORDER BY id DESC");
?>
    ...
    <div class="survey-container">
      ...
      <table>
        ...
        <tbody>
          <?php
          $no = 1;
          $survey = mysqli_query($conn, 'SELECT * FROM `survey_new` LEFT JOIN user ON survey_new.user_id = user.id');
          while ($surveyData = mysqli_fetch_assoc($survey)):
          ?>
            <tr style="font-size: 12px;">
              <td><?= $no++; ?></td>
              <td><?= htmlspecialchars($surveyData['username']); ?></td>
              <td><?= mapping($surveyData['kategori1']); ?></td>
              <td><?= mapping($surveyData['kategori2']); ?></td>
              <td><?= mapping($surveyData['kategori3']); ?></td>
              <td><?= mapping($surveyData['kategori4']); ?></td>
              <td><?= mapping($surveyData['kategori5']); ?></td>
              <td>
